...
void motor_speed_adjust( byte motor, int val ) {
...
void motor_calc_pulse_len(byte motor, unsigned int m_speed, boolean ignore_cal) {
    ...
    // float periods = 1000000.0 / (float) m_min_pulse[motor] / (float) 10.0;
    float periods = 1333; // 25 on/off cycles per second
    
    on_pct[motor]  = ( periods  * m_pct );

    if( ! ignore_cal ) {
        // calibrate
     on_pct[motor] *= motor_cal_adjust(1,motor,m_speed, m_wasdir[motor]);
    }
    
    off_pct[motor] = periods - on_pct[motor];
   ...
   if ( on_pct[motor] != 0 && on_pct[motor] < m_min_pulse[motor] ) {
     ...
     float diff = (float) (m_min_pulse[motor] - on_pct[motor]) / (float) m_min_pulse[motor];
...
void motor_pulse() {
  ...
  volatile static int pulses[2]  = {1,1};
...
float motor_cal_adjust(byte type, byte motor, byte m_spd, byte dir) {
  ...
  byte cal_diff = MOTOR_SPD_CAL_1 - MOTOR_SPD_CAL_0;
  byte hi_diff  = 255 - MOTOR_SPD_CAL_1;
  
  if ( m_spd > MOTOR_SPD_CAL_0 && m_spd < MOTOR_SPD_CAL_1 ) {
    unsigned int diff = m_spd - MOTOR_SPD_CAL_0;
  ...
  else if( m_spd > MOTOR_SPD_CAL_1 ) {
      // between last cal point and max speed
    unsigned int diff = m_spd - MOTOR_SPD_CAL_1;
    float diff_pct = (float) diff / (float) hi_diff;
    float ret = ( m_cal_array[motor][m_angle[motor]][2][dir] * diff_pct ) + (1.0 - diff_pct);
    return(ret);
  }
  else if( m_spd <= MOTOR_SPD_CAL_0 ) {
    pos = 1;
  }
  else if( m_spd >= MOTOR_SPD_CAL_1 ) {
    pos = 2;
...
void motor_execute_ramp_changes() {
    ...
    if( m_ramp_set[m] == 0 )
      ...
      // ramp up?
    if( m_ramp_set[m] >= shots ) {
        // if ramping less than once per shot 
      if( m_ramp_mod[m] > 0 && shots % m_ramp_mod[m] == 0 ) {
        motor_set_speed(m, m_speeds[m] + 1);
      }
      else if( m_ramp_mod[m] == 0 ) {  
        motor_set_speed(m, (m_ramp_shift[m] * shots) );
      }
    }
    else if( (cam_max - shots) <= m_ramp_set[m] ) {
        // ramping down, it seems
      if( m_ramp_mod[m] > 0 && (cam_max - shots) % m_ramp_mod[m] == 0 ) {
        byte m_spd = m_speeds[m] > 0 ? m_speeds[m] - 1 : 0;
        motor_set_speed(m, m_spd);
      }
      else if( m_ramp_mod[m] == 0 ) {  
        motor_set_speed(m, m_ramp_shift[m] * (cam_max - shots) );
      }
    }
  }
