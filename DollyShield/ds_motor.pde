...
void motor_set_speed( byte motor, unsigned int m_speed ) {
  ...
  if( m_speed > 0 && m_speed < min_spd[motor]  ) {
      ...
      on_pct[motor]  = 100.0 * (float) m_speed / 255.0;
        // calibrate
      on_pct[motor] *= motor_cal_adjust(1,motor,m_pct * 100, m_wasdir[motor]);

      off_pct[motor] = 100.0 - on_pct[motor];
     ...
     if ( on_pct[motor] != 0 && on_pct[motor] < m_min_pulse[motor] ) {
       ...
       float diff = m_min_pulse[motor] / on_pct[motor]; 
...
float motor_cal_adjust(byte type, byte motor, byte spd_pct, byte dir) {
  ...
  if( spd_pct <= MOTOR_SPD_CAL_0 ) {
    pos = 1;
  }
  else if( spd_pct <= spd_pct <= MOTOR_SPD_CAL_1 ) {
    pos = 2;
  }
  else {
    return(1.0);
  ...
  if( which == 1 ) {
    ...
    return;
       ...
       byte was_off_pct = off_pct[cur_motor];     
        ...
        on_pct[cur_motor]  = 100 * (float) mspd / 255;
        off_pct[cur_motor] = 100 - on_pct[cur_motor];
        
        if(on_pct[cur_motor] < 1)
          on_pct[cur_motor] = 1;
          
        // make sure that we're on for a minimum amount of time
        
       if ( on_pct[cur_motor] != 0 && on_pct[cur_motor] < m_min_pulse[cur_motor] ) {
           // adjust so that off time is increased relative to on time
         float diff = m_min_pulse[cur_motor] / on_pct[cur_motor]; 
         off_pct[cur_motor] = ((float) off_pct[cur_motor] * diff);
         on_pct[cur_motor] = m_min_pulse[cur_motor];
       }
