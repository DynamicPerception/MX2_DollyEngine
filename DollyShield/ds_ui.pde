 ...
 for( byte i = BUT0; i <= BUT4; i++) {
   ...
   if(  bt_press == 1 ) {
     ...
     if( hold_but_tm <= millis() - HOLD_BUT_MS) {
       ...
       if( hold_but_cnt >= 5 ) {
         inp_val_mult = inp_val_mult >= 500 ? 500 : inp_val_mult * HOLD_BUT_VALINC;
         hold_but_cnt = 0;
       }
...
void ui_button_center( boolean held ) {
    ...
    if( ! (ui_ctrl_flags & B01000000) ) {
      ...
      if( (cur_menu == 2 || cur_menu == 3) && cur_pos == 4 ) {
    ...
    if( ui_ctrl_flags & B00000100 ) {
      ...
      if( merlin_flags & B00100000 ) {
        ...
        if( merlin_flags & B01000000 ) {
            ...
            merlin.setSpeed(2, (unsigned long) 1);
            merlin_set_dir(1,1);            
            merlin_run(1);
          }
        ...
        motor_speed_adjust(cur_motor, -1 * inp_val_mult);
        show_manual();
      }
      ...
      if( merlin_flags & B00100000 ) {
         ...
         if( merlin_flags & B10000000 ) {
           merlin_stop(0);
            ...
            merlin.setSpeed(1, (unsigned long) 1);
            merlin_set_dir(0,0);            
            merlin_run(0);
         }
