...
void show_home() {
 ...
 if( m_type[0] == 1 ) {
   display_spd_deg(m_speeds[0], cur_motor);
 }
 else if( ui_motor_display ) {
  // display pct 
   display_spd_ipm(m_speeds[0], cur_motor);
 }
 else {
 ...
 if( m_type[1] == 1 ) {
  ...
  // display pct 
   display_spd_ipm(m_speeds[1], cur_motor);
 }
 else {
...
void execute_calibrate() {
  ...
  for( byte i = 0; i <= 1; i++ ) {
    ...
    m_cal_array[cur_motor][m_cur_cal][0][i] = 1.0 / cur_inp_float;
  ...
  m_smsfx[0] = 0; m_smsfx[1] = 0;
    ...
    // pulse calibration  
  for( byte c = 1; c <= 2; c++ ) {
    byte ths_spd = c == 1 ? MOTOR_SPD_CAL_0 : MOTOR_SPD_CAL_1;
    
    for( byte i = 0; i <= 1; i++ ) {
      float des_ipm = motor_calc_ipm(cur_motor, ths_spd, true);
        // we only run for 30 seconds, so reduce distance travelled
        // (IPM) by half
      des_ipm *= 0.5;
      ...
      m_cal_array[cur_motor][m_cur_cal][c][i] = des_ipm / cur_inp_float;
    }
  }
